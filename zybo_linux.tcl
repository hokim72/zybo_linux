###############################################################################
# Vivado tcl script for building Zybo FPGA in non project mode
#
# Usage:
# vivado -mode tcl -source zybo_linux.tcl
###############################################################################
...
write_hwdef -force -file $path_sdk/zybo_linux.hwdef
...
read_verilog	$path_rtl/system_ps.v
#read_verilog    $path_rtl/axi_wr_fifo.v
read_verilog    $path_rtl/system_pl.v
read_verilog    $path_rtl/zybo_linux_top.v

read_xdc		$path_xdc/zybo_linux.xdc
...
synth_design -top zybo_linux_top -flatten_hierarchy none -bufg 16 -keep_equivalent_registers
...
#write_hwdef              -file    $path_sdk/zybo_linux.hwdef
...
write_bitstream -force $path_out/zybo_linux.bit

################################################################################
# generate system definition
################################################################################

write_sysdef -force -hwdef $path_sdk/zybo_linux.hwdef \
			 -bitfile $path_out/zybo_linux.bit \
			 -file    $path_sdk/zybo_linux.sysdef
